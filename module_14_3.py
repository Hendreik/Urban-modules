...
Создайте Inline меню из 4 кнопок с надписями "Product1", "Product2", "Product3", "Product4". У всех кнопок назначьте
...
"""
@dp.message_handler(text=['Купить2'])
async def get_buying_list2(message):
	kb = InlineKeyboardMarkup(row_width=10)
...
@dp.message_handler(text=['product_buying1'])
async def send_confirm_message1(message):
	await message.answer(texts.sold, reply_markup=menu_back)
...
async def _back(call):
	await call.message.answer("↓↓ ← ", reply_markup=kb_main)
	await call.answer()
